...
async def async_setup_entry(
    ...
    hass.data.setdefault(f'{COMPONENT_DOMAIN}_lock_entities', [])
    hass.data[f'{COMPONENT_DOMAIN}_lock_add_entities'] = async_add_entities
...
async def async_add_virtual_lock(hass, config):
    """Add a virtual lock entity."""
    entity = VirtualLock(config, False)
    _LOGGER.debug(f"adding virtual lock: {entity}")

    async_add_entities = hass.data.get(f'{COMPONENT_DOMAIN}_lock_add_entities')
    if async_add_entities is not None:
        async_add_entities([entity], update_before_add=True)
        hass.data[f'{COMPONENT_DOMAIN}_lock_entities'].append(entity)
    else:
